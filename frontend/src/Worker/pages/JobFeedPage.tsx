...
// --- Sub-Components (JobPostCard and JobFilters remain unchanged) ---
...
const JobFeedPage: React.FC = () => {
    ...
    // ✅ State to hold the worker's actual full name
    const [workerFullName, setWorkerFullName] = useState("Guest Worker");
    // ✅ State to track if the worker ID check is complete
    ...
    // 1. Fetch current Worker ID and NAME based on logged in user
    useEffect(() => {
        const fetchWorkerProfile = async () => {
            const storedUser = localStorage.getItem("currentUser");
            let foundWorkerId: number | null = null;
            let foundWorkerName: string = "Guest Worker"; // Default fallback
            ...
            if (storedUser) {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        if (myProfile) {
                            foundWorkerId = myProfile.workerID;
                            
                            // ✅ FIX: Get FULL NAME from the nested user object
                            if (myProfile.user?.name) {
                                const { firstName, lastName } = myProfile.user.name;
                                foundWorkerName = `${firstName} ${lastName}`;
                            } else {
                                // Fallback to username from localStorage user object
                                foundWorkerName = user.username || "Worker";
                            }
                        }
                    }
                } catch (error) {
            ...
            // Set final states after all checks are done
            setCurrentWorkerId(foundWorkerId);
            setWorkerFullName(foundWorkerName);
            setIsLoadingWorker(false); // Signal that the worker check is complete
        };
        fetchWorkerProfile();
    }, []);
    ...
    const handleApply = async (jobId: string) => {
         // Block interaction until worker ID is verified (prevents "not logged in" timing error)
        if (isLoadingWorker) {
            alert("Verifying your worker profile status. Please try applying again in a moment.");
            return;
        }

        if (!currentWorkerId) {
            // Correct alert for a non-worker user
            alert("Access Denied: Please ensure your user account is registered as a Worker to apply for jobs."); 
            return;
        }
        ...
        try {
            // API call to assign worker (apply)
            ...
            if (response.ok) {
                ...
                alert(`Failed to apply (${response.status}): ${errorText || "The job may already be assigned or an internal error occurred."}`);
            }
        } catch (error) {
    ...
    return (
        <div className="flex flex-col min-h-screen bg-[#F6F6F6] font-sans">
            <div className="fixed top-0 w-full z-40 bg-[#F6F6F6]">
                {/* ✅ FIXED: Passing the dynamically fetched name */}
